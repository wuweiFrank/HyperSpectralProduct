...
public:
	//最终2级数据生产的接口
	virtual long Level2Proc_Interface(const char* pathOrderFile) = 0;
...
public:
	...
	*/
	//根据EO元素计算IGM文件
	virtual long Level2Proc_CalculateIGMFile(int nLines, int nImgWidth, int nInterval, THREEDPOINT *pGoundPt, vector<EO> &pEOElement, float fFov, float fFocalLen, DPOINT &minPt, DPOINT &maxPt, double dB, double dL, bool bInverse);

	//根据EO元素和DEM元素计算IGM文件
	virtual long Level2Proc_CalculateIGMFileDEM(int nLines, int nImgWidth, int nInterval, THREEDPOINT *pGoundPt, vector<EO> &pEOElement, float fFov, float fFocalLen, DPOINT &minPt, DPOINT &maxPt, double dB, double dL, const char* pathDEM, bool bInverse);
	/*
