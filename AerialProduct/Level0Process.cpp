...
//BIL转换为BSQ文件
//在这里用了内存映射发方法，对于高光谱数据数据量远远大于4G，使用文件跳转的方法需要进行多次跳转
//对于超大数据量的文件跳转遍历的效率特别低，因此需要通过其他的方式进行改进提高效率，而使用内存
//映射的方式将文件映射进入内存，然后直接操作效率能够得到极大的提高
long Level0Process::Level0Proc_RawToBSQ(const char* pRawBIL,const char* pD0BSQ)
{
	CPLSetConfigOption("GDAL_FILENAME_IS_UTF8", "NO");	//中文路径
	GDALAllRegister();
	GDALDatasetH m_dataset = GDALOpen(pRawBIL, GA_ReadOnly);
	...
	char path[_MAX_PATH];
	_splitpath_s(pD0BSQ, drive, dir, filename, ext);
...
//获取非均匀性校正参数(事实证明如果实验室定标做的不好的话还不如直接从影像上计算非均匀性校正系数来的准确)
...
//非均匀性校正文件
long Level0Process::Level0Proc_NonUniform(const char* pD0BSQ,const char* pNonUniform,const char* pCalibFile/*=NULL*/,const char* pDarkFile /*= NULL*/)
{
	CPLSetConfigOption("GDAL_FILENAME_IS_UTF8", "NO");	//中文路径
	GDALAllRegister();
	GDALDatasetH m_dataset = GDALOpen(pD0BSQ, GA_ReadOnly);
	...
	GDALDatasetH m_datasetDark = GDALOpen(pNonUniform, GA_ReadOnly);
	...
	if (pCalibFile != NULL && pDarkFile != NULL)
		GetNonUniformParameters(pCalibFile, pDarkFile, param);
	else if(pDarkFile!= NULL)
	{
	...
	GDALDatasetH m_datasetDst = GDALCreate(GDALGetDriverByName("ENVI"), pNonUniform, xsize, ysize, bands, GDT_UInt16, papszOptions);
...
long QPDLevel0Process::Level0Proc_ExtractEvent(const char* pathRawBIL, const char* pathEvent,const int nEventOffset)
{
	CPLSetConfigOption("GDAL_FILENAME_IS_UTF8", "NO");	//中文路径
	GDALAllRegister();
...
//对数据进行转换，因为短波部分有可能反序，所以转换为BSQ的方式可能会有所不同
