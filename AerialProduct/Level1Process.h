...
//1级数据产品生产，1级数据产品生产输入数据为0级产品输出为1级产品
//1级数据产品生产主要包括相对辐射校正，绝对辐射校正，和数据标定（辐亮度转反射率）
class Level1Process
{
public:
	//最终1级产品生产的接口
	//接口输入订单文件，由订单文件解析各个输入和输出参数
	//最后调用1级产品生产各个函数生成最终结果
	virtual long Level1Proc_Interface(const char* pathOrderFile) = 0;
...
public:
	//接口先空着咯
	virtual long Level1Proc_Interface(const char* pathOrderFile){return 0;}
