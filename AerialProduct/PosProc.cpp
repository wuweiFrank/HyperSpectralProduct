...
long GeoPOSProcess::GeoPOSProc_ExtractEO(const char* pPOSFile,const char* pEOFile,int nLines,int nBeginLine,THREEDPOINT THETA,float* fSetupAngle)
{
	...
	nQuarFlag = GeoPOSProc_EOQuadrant(this->m_Geo_Pos.size(), EMMatrix, XYZPnt);  //计算象限失败
...
//从SBET文件中解算POS数据
//SBET文件为二进制文件，每一条记录记录了各种GPS和IMU的信息
//SBET文件记录信息的频率要远高于影像成像频率，因此在实际处理过程中需要根据
//影像没一行成像时间EVENT信息，从SBET信息中进行差值获取影像实际时刻的位置和姿态
	...
	for (int i = 0; i < nLines&&!feof(fpin); i++)
	{
		...
		for(int j=0;j<(int)formatVec.size();++j)
		{
			if(formatVec[j]==0)
				...
				fReadData[formatVec[j]]=fGetData[j+1];//第一个是编号
		}
		if(!m_arc)
		{
			fReadData[1]*=PI/180;
