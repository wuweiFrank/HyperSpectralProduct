...
//解算IGM文件
long Level2Process::Level2Proc_CalculateIGMFile(int nLines, int nImgWidth, int nInterval, THREEDPOINT *pGoundPt, vector<EO> &pEOElement, float fFov, float fFocalLen, DPOINT &minPt, DPOINT &maxPt, double dB, double dL, bool bInverse)
{
	long lError = 0;
	int i = 0, j = 0;
	...
	int nCorEOLine = 0;
	THREEDPOINT XYZPt;
	CoordinateTransBasic m_coordinateT;
	//成图坐标系到地心坐标系
	dEMMatrix[0] = -sin(dL);
...
//存在DEM的条件下解算IGM文件
long Level2Process::Level2Proc_CalculateIGMFileDEM(int nLines, int nImgWidth, int nInterval, THREEDPOINT *pGoundPt, vector<EO> &pEOElement, float fFov, float fFocalLen, DPOINT &minPt, DPOINT &maxPt, double dB, double dL, const char* pathDEM, bool bInverse)
{
	long lError = 0;
	int i = 0, j = 0;
	double dAngle = 0, dPixelIFov = 0;
