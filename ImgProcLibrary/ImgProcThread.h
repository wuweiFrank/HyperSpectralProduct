...
------------------------------------------------------------------------------*/
//这是一个老外写的win32下的线程池
//调用方法有两个：1.函数直接通过RUN函数进行调用 2.将其封装为类进行调用
...
extern class DllExport CThreadPool;

struct DllExport IRunObject
{
	virtual void Run() = 0;
...
typedef struct DllExport UserPoolData
{
	LPVOID pData;
	CThreadPool* pThreadPool;
...
public:
	CThreadPool(int nPoolSize = POOL_SIZE, bool bCreateNow = true, int nWaitTimeForThreadsToComplete = 5);
	virtual ~CThreadPool();
