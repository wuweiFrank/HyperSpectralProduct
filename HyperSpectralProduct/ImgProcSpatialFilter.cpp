#include "stdafx.h"
#include "ImgProcSpatialFilter.h"
...
/********************************************************************************
*函数描述：spFilterCore设置影像滤波核大小和滤波核内容						    *	
*函数参数：float  **core：二维指针，设置的滤波核								*
*		   int   xsize		  ：滤波核的x大小									*
*		   int	 ysize	      ：滤波核的y大小									*
********************************************************************************/
void spFilter::spFilterCore(float** core,int xsize,int ysize)
{
	if (core==NULL)
		return;
	...
	m_Filter_core=new float *[ysize];
	for (int i=0;i<ysize;i++)
	{
		m_Filter_core[i]=new float [xsize];
		memcpy(m_Filter_core[i],core[i],sizeof(float)*xsize);
	}
	m_is_core=true;
}
...
void spFilter::spFilterCore(float* core,int xsize,int ysize)
{
	...
	{
		m_Filter_core[i]=new float [xsize];
		memcpy(m_Filter_core[i],core+i*xsize,sizeof(float)*xsize);
	}
	m_is_core=true;
}
...
/********************************************************************************
*函数描述：spitalFilter		  : GDAL封装的影像空域滤波						    *	
...
********************************************************************************/
void spFilter::spitalFilter(char* pathSrc,char* pathDst,int xpos,int ypos,int xoffset,int yoffset)
{
	//GDAL封装的影像空域滤波
	GDALAllRegister();
	...
	for (int i=0;i<bands;i++)
	{
		...
		spFilterOperation(m_datasrc,width,heigh,xpos,ypos,xoffset,yoffset,m_datadst);
...
void spFilter::spitalFilter(char* pathSrc,char* pathDst)
{
	//GDAL封装的影像空域滤波
	GDALAllRegister();
	GDALDatasetH m_src_dataset, m_dst_dataset;
	...
	for (int i=0;i<bands;i++)
	{
		...
		GDALRasterIO(m_band,GF_Read,0,0,width,heigh,m_datasrc,width,heigh,GDT_Float32,0,0);
		memcpy(m_datadst,m_datasrc,sizeof(float)*width*heigh);
		spFilterOperation(m_datasrc,width,heigh,0,0,width,heigh,m_datadst);
		m_band=GDALGetRasterBand(m_dst_dataset,i+1);
		GDALRasterIO(m_band,GF_Write,0,0,width,heigh,m_datadst,width,heigh,GDT_Float32,0,0);
	}
...
//具体使用滤波核滤波
//Laplace算子滤波
void spFilter::LaplaceFilter(char* pathSrc,char* pathDst,int xpos,int ypos,int xoffset,int yoffset)
{
	m_is_core=true;
	m_Filter_core=new float *[3];
	for (int i=0;i<3;i++)
		m_Filter_core[i]=new float [3];
	m_Filter_core[0][0]=-1.0f;m_Filter_core[0][1]=-1.0f;m_Filter_core[0][2]=-1.0f;
	m_Filter_core[1][0]=-1.0f;m_Filter_core[1][1]=8.0f; m_Filter_core[1][2]=-1.0f;
	m_Filter_core[2][0]=-1.0f;m_Filter_core[2][1]=-1.0f;m_Filter_core[2][2]=-1.0f;
	m_core_xsize=m_core_ysize=3;
	spitalFilter(pathSrc,pathDst,xpos,ypos,xoffset,yoffset);

}
void spFilter::LaplaceFilter(char* pathSrc,char* pathDst)
{
	m_is_core=true;
	m_Filter_core=new float *[3];
	for (int i=0;i<3;i++)
	...
	m_Filter_core[2][0]=-1.0f;m_Filter_core[2][1]=-1.0f;m_Filter_core[2][2]=-1.0f;
	m_core_xsize=m_core_ysize=3;
	spitalFilter(pathSrc,pathDst);
}

//sobel算子滤波
void spFilter::SobelFilter(char* pathSrc,char* pathDst,int xpos,int ypos,int xoffset,int yoffset)
{
	m_is_core=true;
	m_Filter_core=new float *[3];
	for (int i=0;i<3;i++)
		m_Filter_core[i]=new float [3];
	m_Filter_core[0][0]=1.0f;m_Filter_core[0][1]=0.0f;m_Filter_core[0][2]=-1.0f;
	m_Filter_core[1][0]=2.0f;m_Filter_core[1][1]=0.0f; m_Filter_core[1][2]=-2.0f;
	m_Filter_core[2][0]=1.0f;m_Filter_core[2][1]=0.0f;m_Filter_core[2][2]=-1.0f;
	m_core_xsize=m_core_ysize=3;
	spitalFilter(pathSrc,pathDst,xpos,ypos,xoffset,yoffset);
}
void spFilter::SobelFilter(char* pathSrc,char* pathDst)
{
	m_is_core=true;
	m_Filter_core=new float *[3];
	for (int i=0;i<3;i++)
...
//均值滤波
void spFilter::MeanFilter(char* pathSrc,char* pathDst,int xpos,int ypos,int xoffset,int yoffset)
{
	m_is_core=true;
	m_Filter_core=new float *[3];
	for (int i=0;i<3;i++)
		m_Filter_core[i]=new float [3];
	m_Filter_core[0][0]=1.0f/9.0f;m_Filter_core[0][1]=1.0f/9.0f;m_Filter_core[0][2]=1.0f/9.0f;
	m_Filter_core[1][0]=1.0f/9.0f;m_Filter_core[1][1]=1.0f/9.0f;m_Filter_core[1][2]=1.0f/9.0f;
	m_Filter_core[2][0]=1.0f/9.0f;m_Filter_core[2][1]=1.0f/9.0f;m_Filter_core[2][2]=1.0f/9.0f;
	m_core_xsize=m_core_ysize=3;
	spitalFilter(pathSrc,pathDst,xpos,ypos,xoffset,yoffset);
}
void spFilter::MeanFilter(char* pathSrc,char* pathDst)
{
	m_is_core=true;
	m_Filter_core=new float *[3];
	for (int i=0;i<3;i++)
...
//0值滤波
void spFilter::ZeroFilter(char* pathSrc,char* pathDst,int xpos,int ypos,int xoffset,int yoffset)
{
	m_is_core=true;
	m_Filter_core=new float *[3];
	for (int i=0;i<3;i++)
		m_Filter_core[i]=new float [3];
	m_Filter_core[0][0]=0.0f;m_Filter_core[0][1]=0.0f;m_Filter_core[0][2]=0.0f;
	m_Filter_core[1][0]=0.0f;m_Filter_core[1][1]=0.0f;m_Filter_core[1][2]=0.0f;
	m_Filter_core[2][0]=0.0f;m_Filter_core[2][1]=0.0f;m_Filter_core[2][2]=0.0f;
	m_core_xsize=m_core_ysize=3;
	spitalFilter(pathSrc,pathDst,xpos,ypos,xoffset,yoffset);
}
void spFilter::ZeroFilter(char* pathSrc,char* pathDst)
{
	m_is_core=true;
	m_Filter_core=new float *[3];
	for (int i=0;i<3;i++)
...
void spFilter::MedFilter(float*imgBufferIn,int width,int heigh,int xpos,int ypos,int xoffset,int yoffset,int xfsize,int yfsize,float* imgBufferOut)
{
...
/********************************************************************************
*函数描述：spFilterOperation设置影像滤波核大小和滤波核内容					    *	
*函数参数：float  *imgBufferIn：输入影像的数据空间								*
*		   int   width		  ：影像的宽度										*
*		   int	 heigh	      ：影像的高度										*
*		   int	 xpos	      ：滤波影像的起始x位置								*
...
*		   float  *imgBufferOut：输出影像的数据空间								*
********************************************************************************/
void spFilter::spFilterOperation(float*imgBufferIn,int width,int heigh,int xpos,int ypos,int xoffset,int yoffset,float* imgBufferOut)
{
	m_width=width;
	m_heigh=heigh;
	if (xpos<0||ypos<0||xpos>width||ypos>heigh||xpos+xoffset>width||ypos+yoffset>heigh)
	...
	for (int i=xpos;i<xpos+xoffset;i++)
		for (int j=ypos;j<ypos+yoffset;j++)
			imgBufferOut[j*width+i]=spFilterOperationCore(imgBufferIn,i,j);
}
...
*		   int	 ypos	      ：模板处理位置y									*
********************************************************************************/
float __fastcall spFilter::spFilterOperationCore(float*imgBufferIn,int xpos,int ypos)
{
	if (imgBufferIn==NULL)
		return -100000;
	if (xpos<0||ypos<0)
	...
	for (int i=-int(xcoresize/2);i<=int(xcoresize/2);i++)
	{
		for (int j=-int(ycoresize/2);j<=int(ycoresize/2);j++)
		{
			...
			if (tempy>=m_heigh)
				tempy=ypos-j;
			tempData+=imgBufferIn[tempy*m_width+tempx]*m_Filter_core[j+int(ycoresize/2)][i+int(xcoresize/2)];
		}
	}
	return abs(tempData);
}
...
/********************************************************************************
*函数描述：GaussFilter设置影像gauss滤波核大小和滤波核内容					    *	
...
*		   int	 ykernel	  ：滤波核的y大小									*
********************************************************************************/
long GaussFilter(float *srcImg,float *dstImg,int xImg,int yImg,float ** kernel,int xkernel,int ykernel)
{
	//检查输入
...
//GDAL封装高斯滤波
long  GaussFilter(char* pathSrc,char* pathDst,float** kernel, int xkernel, int ykernel)
{
	//检查输入
	...
	for (int i=0;i<band;i++)
	{
		...
		GaussFilter(datasrc,datadst,xImg,yImg,kernel,xkernel,ykernel);
...
//影像分块空域滤波
long ImageSpaitalBlockFilter(char* pathsrc,char* pathdst,float* FilterCore,int xcoresize,int ycoresize)
{
	GDALAllRegister();
	...
	//分块滤波并写入文件中
	spFilter spatialFilter;spatialFilter.spFilterCore(FilterCore,xcoresize,ycoresize);
	for (int i=0;i<xblock*yblock;i++)
	{
		...
		for (int j=0;j<bands;j++)
		{
			...
			spatialFilter.spFilterOperation(dataIn,readWidth,readHeight,0,0,readWidth,readHeight,dataOut);
...
//多线程空域滤波线程函数
DWORD WINAPI thread_spatial_Filter_core(LPVOID dataStruct) 
{
	UserPoolData* poolData = (UserPoolData*)dataStruct;
	spFilterThreadStruct* sampleSt=(spFilterThreadStruct*)poolData->pData;
	spFilter spatialFilter;
	//数据
	...
	float* coredata=sampleSt->Filter_core;
	...
	float* dataIn =new float[readWidth*readHeight];memset(dataIn,0,sizeof(float)*readHeight*readWidth);
	float* dataOut=new float[readWidth*readHeight];memset(dataOut,0,sizeof(float)*readHeight*readWidth);
	spatialFilter.spFilterCore(coredata,m_corexsize,m_coreysize);
...
//多线程空域滤波
long ThreadImageSpaitalFilter(char* pathsrc,char* pathdst,float* FilterCore,int xcoresize,int ycoresize)
{
	GDALAllRegister();
	GDALDatasetH m_dataset_src;
	GDALDatasetH m_dataset_dst;
	spFilterThreadStruct* threadSt;
	...
	threadSt=new spFilterThreadStruct[xblock*yblock*bands];
	//多线程
	...
	for (int i=0;i<xblock*yblock;i++)
	{
		for (int j=0;j<bands;j++)
		{
			...
			threadSt[j*xblock*yblock+i].Filter_core=FilterCore;
			threadSt[j*xblock*yblock+i].image_band=j+1;
			m_threadpool.Run(thread_spatial_Filter_core,&threadSt[j*xblock*yblock+i],Low);
		}
	}
...
********************************************************************************/
//设置滤波核
void MorphologyFilter::MorphologyCore(float** core,int xsize,int ysize,int centerx,int centery)
{
	if (core==NULL)
		return;
...
//形态学腐蚀操作
void MorphologyFilter::MorphologyCorrode(float* dataIn,int sizex,int sizey,int bands,float* dataOut)
{
	if(dataIn==NULL||dataOut==NULL)
		return ;
	if(!this->m_is_core)
	{
		printf("do not set Filter core!\n");
		return ;
	}
	//腐蚀操作滤波核窗口内最小的像素
	...
	for (int i=0;i<sizex;i++)
	{
		for (int j=0;j<sizey;j++)
		{
			...
			for (int l=0;l<m_core_xsize;l++)
			{
				...
				{
					if(this->m_Filter_core[m*this->m_core_xsize+l]!=0)
...
void MorphologyFilter::MorphologyDilation(float* dataIn,int sizex,int sizey,int bands,float* dataOut)
{
	if(dataIn==NULL||dataOut==NULL)
		return ;
	if(!this->m_is_core)
...
//GDAL封装的形态学腐蚀与膨胀
//形态学腐蚀
void MorphologyFilter::MorphologyCorrode(char* pathIn,char* pathOut)
{
	if(!this->m_is_core)
	{
		printf("do not set Filter core!\n");
...
//形态学膨胀
void MorphologyFilter::MorphologyDilation(char* pathIn,char* pathOut)
{
	if(!this->m_is_core)
	{
		printf("do not set Filter core!\n");
